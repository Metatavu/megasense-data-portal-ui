...
class MapScreen extends React.Component<Props, State> {
  ...
  public componentDidMount = async () => {
    ...
    const airQuality = await airQualityApi.getAirQuality({
      pollutant: "SULFUR_DIOXIDE",
      boundingBoxCorner1: "55,20",
      boundingBoxCorner2: "65,30",
    });
  ...
  private displaySavedRoute = (routeToDisplay: Route) => {
    ...
    const locationFrom = {
      coordinates: locationFromCoordinates,
      name: routeToDisplay.locationFromName,
    };

    const locationToCoordinates = `${lastItem[0]},${lastItem[1]}`;
    const locationTo = {
      coordinates: locationToCoordinates,
      name: routeToDisplay.locationToName,
    };

    const newCenterCoordinates = this.coordinatesFromString(
    ...
    const newState = {
      ...
      mapViewport: {
        center: [newCenterCoordinates[0], newCenterCoordinates[1]] as [
          number,
          number
        ],
        zoom: 13,
  ...
  private loadUserSettings = async () => {
    ...
    try {
      ...
      if (homeAddress) {
        ...
        const nominatimResponse: Nominatim.NominatimResponse[] = await Nominatim.geocode(
          geocodeRequest,
          process.env.REACT_APP_NOMINATIM_URL
        );
        if (nominatimResponse.length === 1) {
          const center = [
            Number.parseFloat(nominatimResponse[0].lat),
            Number.parseFloat(nominatimResponse[0].lon),
          ] as [number, number];
  ...
  private onDeleteUserSavedRoute = async (routeId: string) => {
    ...
    this.setState({
      userSavedRoutes: userSavedRoutes.filter(
        (userSavedRoute) => userSavedRoute.id !== routeId
      ),
    });
  };
  ...
  private onDeleteUserFavouriteLocation = async (locationId: string) => {
    ...
    this.setState({
      userFavouriteLocations: userFavouriteLocations.filter(
        (userSavedLocation) => userSavedLocation.id !== locationId
      ),
    });
  };
            ...
            endIcon={
              loadingRoute ? (
                <CircularProgress
                  size={20}
                  color="inherit"
                  className={classes.routingFormLoader}
                />
              ) : (
    ...
    return (
      ...
      >
        ...
        {selectedFavouriteLocation?.coordinates && (
          <Marker
            ref={this.openPopup}
            position={this.coordinatesFromString(
              selectedFavouriteLocation.coordinates
            )}
          >
            <Popup
              ...
              autoPan={false}
            >
              <h3>{selectedFavouriteLocation.name}</h3>
              <Button
                onClick={this.onSetSourceClick(
                  selectedFavouriteLocation.coordinates,
              ...
              <Button
                onClick={this.onSetDestinationClick(
                  selectedFavouriteLocation.coordinates,
                  selectedFavouriteLocation.name
                )}
              >
        ...
        {locationFrom?.coordinates && (
          <Marker
            position={this.coordinatesFromString(locationFrom.coordinates)}
          >
            <Popup
              onOpen={this.switchMapInteraction}
            ...
            >
              <h3>{`${strings.from}:`}</h3>
              <p>{locationFromTextInput || ""}</p>
              <Button
                onClick={() =>
                  this.onSaveLocationClick(locationFrom.coordinates)
                }
              >
        ...
        {locationTo?.coordinates && (
          <Marker position={this.coordinatesFromString(locationTo.coordinates)}>
            ...
            >
              <h3>{`${strings.to}:`}</h3>
              <p>{locationToTextInput || ""}</p>
              <Button
                onClick={() => this.onSaveLocationClick(locationTo.coordinates)}
              >
                {strings.locations.saveLocation}
              </Button>
            </Popup>
        ...
        {this.state.mapViewport.zoom === 6 && (
          <HeatmapLayer
            points={airQuality}
            longitudeExtractor={(airQuality: AirQuality) =>
              airQuality.location.longitude
            }
            latitudeExtractor={(airQuality: AirQuality) =>
              airQuality.location.latitude
            }
            intensityExtractor={(airQuality: AirQuality) =>
              airQuality.pollutionValue
            }
          />
        )}
      </Map>
   ...
   */
  private onDialogNameInputChange = (
    action: React.ChangeEvent<HTMLTextAreaElement | HTMLInputElement>
  ) => {
    this.setState({
  ...
  private onLocationSaveConfirm = async () => {
    ...
    try {
      const locationsApi = Api.getLocationsApi(accessToken);
      const [latitude, longitude] = this.coordinatesFromString(
        savingLocationCoordinates
      );
   ...
   */
  private onLocationFromSelected = (
    event: React.ChangeEvent<{}>,
    locationFrom: Location | null | undefined,
   ...
   * @param reason autocomplete change reason
   */
  private onLocationFromChange = async (
    event: ChangeEvent<{}>,
    locationFromTextInput: string,
    ...
    try {
      const nominatimResponse: Nominatim.NominatimResponse[] = await Nominatim.geocode(
        { email: "devs@metatavu.fi", q: locationFromTextInput },
        process.env.REACT_APP_NOMINATIM_URL
      );
   ...
   * @param reason autocomplete change reason
   */
  private onLocationToSelected = (
    event: React.ChangeEvent<{}>,
    locationTo: Location | null | undefined,
    reason: AutocompleteChangeReason
  ) => {
    if (locationTo === null) {
   ...
   * @param reason autocomplete change reason
   */
  private onLocationToChange = async (
    event: ChangeEvent<{}>,
    locationToTextInput: string,
    reason: AutocompleteInputChangeReason
  ) => {
    ...
    try {
      const nominatimResponse: Nominatim.NominatimResponse[] = await Nominatim.geocode(
        { email: "devs@metatavu.fi", q: locationToTextInput },
        process.env.REACT_APP_NOMINATIM_URL
      );
  ...
  private updateRoute = async () => {
    ...
    try {
      ...
      const routingResponse = await fetch(
        `${process.env.REACT_APP_OTP_URL}?fromPlace=${
          locationFrom?.coordinates
        }&toPlace=${locationTo?.coordinates}&time=${moment(
          departureTime
        ).format("h:mma")}&date=${moment(departureTime).format(
          "MM-DD-yyyy"
        )}&maxWalkDistance=100000&sulfurDioxideThreshold=1&sulfurDioxidePenalty=200`
      );
      const jsonResponse = await routingResponse.json();
      const polyline =
        jsonResponse.plan.itineraries[0].legs[0].legGeometry.points;
      ...
      this.setState({
        ...
        locationTo,
        loadingRoute: false,
        mapViewport: { center: newCenter as [number, number], zoom: 13 },
        previousZoom: 13,
        polyline,
      });
    } catch (error) {
  ...
  private addRoutePoint = async (mouseEvent: LeafletMouseEvent) => {
    const position =
      mouseEvent.latlng.lat.toString() + "," + mouseEvent.latlng.lng.toString();
    const location = { name: position, coordinates: position };
    if (this.state.editingLocationFrom) {
      const locationFromOptions = [location];
      const nominatimResponse: Nominatim.NominatimResponse = await Nominatim.reverseGeocode(
        {
          lat: mouseEvent.latlng.lat.toString(),
          lon: mouseEvent.latlng.lng.toString(),
        },
        process.env.REACT_APP_NOMINATIM_URL
      );
      if (nominatimResponse) {
        this.setState({
          ...
          locationFromTextInput: nominatimResponse.display_name,
        });
      }
    } else {
      const nominatimResponse: Nominatim.NominatimResponse = await Nominatim.reverseGeocode(
      ...
      if (nominatimResponse) {
        this.setState({
          locationTo: location,
          editingLocationFrom: true,
          locationToTextInput: nominatimResponse.display_name,
        });
      }
    }
  };
   ...
   */
  private reverseGeocodeCoordinates = async (
  ...
  private onUserRouteSelect = (route: Route) => {
    ...
    this.reverseGeocodeCoordinates(
      GeocodeCoordinate.From,
      sourceCoordinates[0],
      sourceCoordinates[1]
    );
    this.reverseGeocodeCoordinates(
      GeocodeCoordinate.To,
      destCoordinates[0],
      destCoordinates[1]
    );
